...
export const AdminDashboard: React.FC = () => {
  ...
  const chartData = (stats?.weeklyTrend && stats.weeklyTrend.length > 0)
    ? stats.weeklyTrend.map((item: any) => ({
        ...item,
        formattedDate: new Date(item.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
      }))
  ...
  const pieData = [
    { name: 'Pending Expenses', value: stats?.expensesSummary?.pending || 0, color: '#f59e0b' },
    { name: 'Approved Claims', value: stats?.expensesSummary?.approved || 0, color: '#10b981' },
    { name: 'Rejected Claims', value: stats?.expensesSummary?.rejected || 0, color: '#ef4444' }
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200/70 shadow-2xs">
            ...
            {/* Recharts Multi-View Analytics Chart */}
              ...
              {activeChartView === 'area' && (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <XAxis dataKey="formattedDate" tick={{ fontSize: 11, fill: '#64748b', fontWeight: 600 }} />
                    <YAxis tick={{ fontSize: 11, fill: '#64748b', fontWeight: 600 }} unit=" KM" />
                    <Tooltip
                      contentStyle={{
                      ...
                      formatter={(value: any) => [`${value} KM`, 'Distance Travelled']}
                    />
                    <Area
                      type="monotone"
                      dataKey="km"
                ...
                <div className="flex flex-col sm:flex-row items-center justify-around h-full gap-4">
                  <div className="w-full sm:w-1/2 h-48 sm:h-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={pieData.every((d) => d.value === 0) ? [{ name: 'No Claims', value: 1, color: '#cbd5e1' }] : pieData}
                          innerRadius={50}
                          outerRadius={80}
                          paddingAngle={4}
                          dataKey="value"
                        >
                          {(pieData.every((d) => d.value === 0) ? [{ name: 'No Claims', value: 1, color: '#cbd5e1' }] : pieData).map((entry, index) => (
                  ...
                  <div className="w-full sm:w-1/2 space-y-2.5 text-xs font-bold text-slate-700 sm:border-l border-slate-100 sm:pl-6">
                    <div className="flex items-center justify-between p-2 rounded-lg bg-amber-50/60 border border-amber-100">
                      <span className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full bg-amber-500 inline-block" />
                        Pending Claims:
                      </span>
                      <span className="font-black text-amber-700">{stats?.expensesSummary?.pending || 0}</span>
                    </div>
                    <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-50/60 border border-emerald-100">
                      ...
                      <span className="font-black text-emerald-700">{stats?.expensesSummary?.approved || 0}</span>
                    </div>
                    <div className="flex items-center justify-between p-2 rounded-lg bg-red-50/60 border border-red-100">
                      ...
                      <span className="font-black text-red-700">{stats?.expensesSummary?.rejected || 0}</span>
                    </div>
                  </div>
                </div>
              )}
